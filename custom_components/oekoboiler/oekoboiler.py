...
def processImage(im):
    ...
    # Time
    img_time = cropToBoundry(im, BOUNDRY_TIME)
    opencv_time = cv.cvtColor(numpy.array(img_time), cv.COLOR_RGB2BGR)
    cnts, digits, value = findDigits(opencv_time, "Time")
    time = "{}{}:{}{}".format(digits[0],digits[1],digits[2],digits[3])
    print("Zeit Value: {}".format(time))


    # Set Temperature 
    img_setTemp = cropToBoundry(im, BOUNDRY_SETTEMP)
    opencv_setTemp = cv.cvtColor(numpy.array(img_setTemp), cv.COLOR_RGB2BGR)
    cnts, digit, value = findDigits(opencv_setTemp, "Set Temp")
    print("Set Temperature Value: {}".format(value))


    # Water Temperature
    img_waterTemp = cropToBoundry(im, BOUNDRY_WATERTEMP)
    opencv_waterTemp = cv.cvtColor(numpy.array(img_waterTemp), cv.COLOR_RGB2BGR)
    cnts, digits,value = findDigits(opencv_waterTemp)
    print("Water Temperature Value: {}".format(value))


    # Modus
    img_modeAuto = cropToBoundry(im, BOUNDRY_MODE_AUTO)
    opencv_modeAuto = cv.cvtColor(numpy.array(img_modeAuto), cv.COLOR_RGB2BGR)
    modeAuto = isIlluminated(opencv_modeAuto)
    print ("Econ Auto: {}".format(modeAuto))

    img_modeEcon = cropToBoundry(im, BOUNDRY_MODE_ECON)
    opencv_modeEcon = cv.cvtColor(numpy.array(img_modeEcon), cv.COLOR_RGB2BGR)
    modeEcon = isIlluminated(opencv_modeEcon)
    print ("Econ Mode: {}".format(modeEcon))

    img_modeHeater = cropToBoundry(im, BOUNDRY_MODE_HEATER)
    opencv_modeHeater = cv.cvtColor(numpy.array(img_modeHeater), cv.COLOR_RGB2BGR)
    modeHeater = isIlluminated(opencv_modeHeater)
    print ("Econ Heater: {}".format(modeHeater))

    # Indicators
    img_warmIndicator = cropToBoundry(im, BOUNDRY_INDICATOR_WARM, removeBlue=True)
    opencv_warmIndicator= cv.cvtColor(numpy.array(img_warmIndicator), cv.COLOR_RGB2BGR)
    warmIndicator = isIlluminated(opencv_warmIndicator, "warm")
    print ("warmIndicator: {}".format(warmIndicator))

    img_defIndicator = cropToBoundry(im, BOUNDRY_INDICATOR_DEF, removeBlue=True)
    opencv_defIndicator= cv.cvtColor(numpy.array(img_defIndicator), cv.COLOR_RGB2BGR)
    defIndicator = isIlluminated(opencv_defIndicator, "def")
    print ("defIndicator: {}".format(defIndicator))

    img_htgIndicator = cropToBoundry(im, BOUNDRY_INDICATOR_HTG, removeBlue=True)
    opencv_htgIndicator= cv.cvtColor(numpy.array(img_htgIndicator), cv.COLOR_RGB2BGR)
    htgIndicator = isIlluminated(opencv_htgIndicator, "htg")
    print ("htgIndicator: {}".format(htgIndicator))

    img_offIndicator = cropToBoundry(im, BOUNDRY_INDICATOR_OFF, removeBlue=True)
    opencv_offIndicator= cv.cvtColor(numpy.array(img_offIndicator), cv.COLOR_RGB2BGR)
    offIndicator = isIlluminated(opencv_offIndicator, "off")
    print ("offIndicator: {}".format(offIndicator))


    
    
    opencv_image = cv.cvtColor(numpy.array(im), cv.COLOR_RGB2BGR)
...
def isIlluminated(im, title=""):
    ...
    if DRAW_ILLUMINATION and nonZeroValue > threshold:
        ...
        h, w = im.shape[:2]
        im = cv.rectangle(im,(0,0),(w,h),(0,255,0),11)

    return nonZeroValue > threshold


def findDigits(im, title=""):
    gray = cv.cvtColor(im, cv.COLOR_BGR2GRAY)
...
def cropToBoundry(im, boundry, convertToGray=True, removeBlue=False):
