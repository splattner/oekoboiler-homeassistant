...
async def async_update_options(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    boundries = {
        "time": tuple(map(int, entry.option[CONF_BOUNDRY_TIME].split(', '))),
        "setTemp": tuple(map(int, entry.option[CONF_BOUNDRY_SETTEMP].split(', '))),
        "waterTemp": tuple(map(int, entry.option[CONF_BOUNDRY_WATERTEMP].split(', '))),
        "modeAuto": tuple(map(int, entry.option[CONF_BOUNDRY_MODE_AUTO].split(', '))),
        "modeEcon": tuple(map(int, entry.option[CONF_BOUNDRY_MODE_ECON].split(', '))),
        "modeHeater": tuple(map(int, entry.option[CONF_BOUNDRY_MODE_HEATER].split(', '))),
        "indicatorWarm": tuple(map(int, entry.option[CONF_BOUNDRY_INDICATOR_WARM].split(', '))),
        "indicatorOff": tuple(map(int, entry.option[CONF_BOUNDRY_INDICATOR_OFF].split(', '))),
        "indicatorHtg": tuple(map(int, entry.option[CONF_BOUNDRY_INDICATOR_HTG].split(', '))),
        "indicatorDef": tuple(map(int, entry.option[CONF_BOUNDRY_INDICATOR_DEF].split(', '))),
    }
